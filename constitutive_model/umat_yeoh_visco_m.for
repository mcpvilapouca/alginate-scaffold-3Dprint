...
C     24/05/2022  Maria Vila Pouca     Visco only for matrix          |
C     10/08/2022  Maria Vila Pouca     Adapted matrix for yeoh model  |
C--------------------------------------------------------------------
C     Description:
C     UEXTERNALDB: mount database; fibers directions;.inc, .inp files
C     UMAT: NH matrix + Holzapfel fibers + viscoelasticity
C--------------------------------------------------------------------
...
C                COUNTERS AND STATE INDICATORS
     ...
     1         FLAG1,FLAG2,ALPHA,IALPHA,ISTATC,ISTATCBAR
C
...
C                MATERIAL TANGENT VARIABLES
     ...
     9        DDSPKDDE(3,3,3,3),IIII(3,3,3,3),CCBAR_M(3,3,3,3)
C
C                MATERIAL PARAMETERS VARIABLES
C                MECHANICAL MODEL
      DOUBLE PRECISION  D1,C10,C20,C30,K11,K12,K21,K22,
C
...
C                STRAIN-ENERGY DERIVATIVES VARIABLES
     6        DUDI1, D2UDI1,DUDJ, DUDST4, DUDST6, D2UDST4, D2UDST6, D2DJ
C----------------------------------------------------------------------
...
c      IF (TIME(1).EQ.0.D0.AND.KSTEP.EQ.1) THEN
      IF (KSTEP.EQ.1) THEN
C       VISCO
        DO I=1,27
        STATEV(I)=0.D0
...
C     MATRIX CONTRIBUTION
      DUDI1=C10+C20*(TWO*I1BAR-SIX)+THREE*C30*((I1BAR-THREE)**TWO)
      D2UDI1=TWO*C20+SIX*C30*(I1BAR-THREE)
...
C     MATERIAL MATRIX(CCBAR)
      CALL TENSORPROD2(UNIT,UNIT,IIII,NDI)
      ...
      DO I=1,NDI
       DO J=1,NDI
         DO K=1,NDI
          DO L=1,NDI
          CCBAR_M(I,J,K,L)=FOUR*J43*(D2UDI1)*IIII(I,J,K,L)
...
C
C     MATRIX
      CALL DOUBLEDOT4(PP,CCBAR_M,AAUX_M,NDI)
      CALL DOUBLEDOT4(AAUX_M,PPT,CCISO1_M,NDI)
C
      CALL DOUBLEDOT2(SBARM,C,ESC_M,NDI)
      CALL TENSORPROD2(CINV,SISOM,CCISO2_M,NDI)
      CALL TENSORPROD2(SISOM,CINV,CCISO3_M,NDI)

      CCISO_M=CCISO1_M+(TWO/THREE)*J23*ESC_M*PPMOD-(TWO/THREE)*
     *        (CCISO2_M+CCISO3_M)
C
C     FIBER 1
